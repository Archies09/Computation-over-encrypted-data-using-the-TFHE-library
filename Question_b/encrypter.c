...
int main()
{
    ...
    for(int i=0;i<10001;i++)
    {
        ...
        bootsSymEncrypt(encrypted_count[i],0,secret_key);
        for(int j=0;j<32;j++)
        {
            bootsSymEncrypt(&encrypted_total_cost[i][j],(total_costs[i]>>j)&1,secret_key);
        }

    }

    encrypted_threshold = new_gate_bootstrapping_ciphertext_array(32, params);
    for (int i=0; i<32; i++)
    {
        bootsSymEncrypt(&encrypted_threshold[i], (threshold>>i)&1,secret_key);
    }
    ...
    for(int i=0;i<10001;i++)
    {
        for(int j=0;j<32;j++)
        {
            export_gate_bootstrapping_ciphertext_toFile(cloud_data_file,&encrypted_total_cost[i][j],params);
        }
        export_gate_bootstrapping_ciphertext_toFile(cloud_data_file,encrypted_count[i],params);
    }
    for(int j=0;j<32;j++)
    {
        export_gate_bootstrapping_ciphertext_toFile(cloud_data_file,&encrypted_threshold[j],params);
    }
    ...
    for(int i=0;i<10001;i++)
    {
        delete_gate_bootstrapping_ciphertext_array(32,encrypted_total_cost[i]);
        delete_gate_bootstrapping_ciphertext_array(1,encrypted_count[i]);

    }
    delete_gate_bootstrapping_ciphertext_array(32,encrypted_threshold);
